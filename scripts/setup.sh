...
ensure_split_database_secrets() {
  local app_password runtime_password temporary
  app_password="$(env_value FORGE_APP_DATABASE_PASSWORD)"
  runtime_password="$(env_value FORGE_RUNTIME_API_DATABASE_PASSWORD)"
  [ -n "$app_password" ] || app_password="$(random_hex 16)"
  [ -n "$runtime_password" ] || runtime_password="$(random_hex 16)"
  grep -q '^FORGE_APP_DATABASE_PASSWORD=' "$ENV_FILE" || printf 'FORGE_APP_DATABASE_PASSWORD=%s\n' "$app_password" >> "$ENV_FILE"
  grep -q '^FORGE_RUNTIME_API_DATABASE_PASSWORD=' "$ENV_FILE" || printf 'FORGE_RUNTIME_API_DATABASE_PASSWORD=%s\n' "$runtime_password" >> "$ENV_FILE"
  temporary="${ENV_FILE}.forge-setup.$$"
  sed "s|^DATABASE_URL=.*|DATABASE_URL=postgresql://forge:${app_password}@localhost:5432/forge|" "$ENV_FILE" > "$temporary"
  chmod 600 "$temporary"
