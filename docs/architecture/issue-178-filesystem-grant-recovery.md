...
6. **One lock order.** S3 uses the prefix project → tasks ascending → packages
   ...
   agent run → runtime audit claim → packet artifact → review-gate rows ascending.
...
claim → packet artifact → review-gate rows ascending.
...
9. The full project → tasks → packages → approval → protocol epoch → agent run →
   audit → artifact → review-gate rows order has
