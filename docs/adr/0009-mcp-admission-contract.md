...
- **Global lock order.** S3 uses this prefix:
  ...
  worker-protocol epoch → agent run → runtime audit claim → packet artifact →
  review-gate rows ascending. S3 does not acquire the epoch row. Candidate discovery may
