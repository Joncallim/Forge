-- Forward-only repair for the exact legacy Epic 172 0023/0025 catalog
-- fingerprint. The caller owns the transaction and verifies the ledger and
-- physical preconditions before executing this artifact. Do not run old migrations
-- or modify drizzle.__drizzle_migrations here.

ALTER TABLE public.forge_epic_172_release_evidence
  ADD COLUMN required_evidence jsonb NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE public.forge_epic_172_release_evidence
  ALTER COLUMN required_evidence DROP DEFAULT,
  DROP CONSTRAINT forge_epic_172_release_evidence_sha_chk,
  ADD CONSTRAINT forge_epic_172_release_evidence_required_evidence_chk
    CHECK (pg_catalog.jsonb_typeof(required_evidence) = 'array'
      AND pg_catalog.jsonb_array_length(required_evidence) > 0),
  ADD CONSTRAINT forge_epic_172_release_evidence_sha_chk
    CHECK (reviewed_sha ~ '^([0-9a-f]{40}|[0-9a-f]{64})$');
...
ALTER TABLE public.forge_epic_172_enablement_state
  ...
  ADD CONSTRAINT forge_epic_172_enablement_sha_chk
    CHECK (reviewed_sha IS NULL OR reviewed_sha ~ '^([0-9a-f]{40}|[0-9a-f]{64})$');
...
FROM forge_release_evidence_consumer;
...
-- The legacy release routine identities must not survive alongside the fixed
-- signatures below.
DROP FUNCTION forge.record_epic_172_release_evidence_v1(
  uuid,text,integer,text,jsonb,text,bigint,jsonb,text,text,uuid,bigint,
