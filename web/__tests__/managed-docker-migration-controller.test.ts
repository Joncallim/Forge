...
describe('managed Docker migration authority', () => {
  ...
  it('passes the ephemeral credential only to the child and cleans it up on all outcomes', () => {
    ...
    expect(controller).toContain('lock table drizzle.__drizzle_migrations in access exclusive mode')
    expect(controller).toContain('alter schema drizzle owner to forge_schema_owner')
    expect(controller).toContain('exact journal prefix')
    expect(controller).toContain('pg_catalog.pg_event_trigger')
    expect(controller.indexOf("refused enabled PostgreSQL event triggers before controller DDL"))
      .toBeLessThan(controller.indexOf("if not exists(select 1 from pg_roles where rolname='forge_schema_owner')"))
    expect(controller.indexOf('normalizeExistingMigrationLedger(sql, expectedLedger, targetEntry.when, true)'))
      .toBeLessThan(controller.indexOf('prepareProtectedMigrationController(sql'))
    expect(controller).not.toContain('select exists(select 1 from drizzle.__drizzle_migrations')
    expect(controller).toContain('FORGE_MANAGED_MIGRATION_PAUSE_LEDGER_OWNER_TRANSFER_MS')
    expect(controller).not.toMatch(/grant .*drizzle.* to \$\{safe\(migrat/i)
