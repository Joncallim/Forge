...
describe('project-root expansion reconciliation boundary', () => {
  ...
  it('proves the exact closed privilege allowlist rejects direct, column, and PUBLIC-effective mutations', () => {
    ...
    expect(privilegeMutationProof).toContain('SELECT session_user AS mutation_session_user, current_user AS mutation_current_user \\gset')
    expect(privilegeMutationProof).toContain('ROOT_GRANT_OPTION_MUTATION_IDENTITY session=:mutation_session_user current=:mutation_current_user')
    expect(privilegeMutationProof).toContain('PostgreSQL records ACL grantors as current_user')
    ...
    expect(privilegeMutationProof).toContain('public.projects:SELECT:grantor=${mutation_current_user}:grantee=forge_project_root_reconciler')
