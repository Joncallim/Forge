...
export async function GET(
  ...
  try {
    ...
    for (const d of decisions) {
      if (!d.workPackageId) continue
      const list = decisionsByPackage.get(d.workPackageId) ?? []
      list.push(d)
      decisionsByPackage.set(d.workPackageId, list)
    }
    ...
    const grants = packages.map((pkg) => {
      ...
      return {
        ...
        decisionHistory: history.map((d) => ({
          id: d.id, decision: d.decision, capabilities: d.capabilities,
          grantDecisionRevision: d.grantDecisionRevision?.toString() ?? null,
          rootBindingRevision: d.rootBindingRevision?.toString() ?? null,
    ...
    const presenter = {
      ...
      freshnessFingerprint: computeFreshnessFingerprint({ taskId, grantIds: decisions.map((d) => d.id) }),
      taskId, grants,
    }
