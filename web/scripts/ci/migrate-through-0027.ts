...
const LAST_BASELINE_MIGRATION = '0027_epic_172_s4_packet_context'
const NEXT_MIGRATION = '0028_epic_172_s5_recovery_actions'
...
async function main(): Promise<void> {
  ...
  if (
    ...
    throw new Error('0028 must immediately and chronologically follow 0027 in the migration journal.')
  }

  const temporaryRoot = await mkdtemp(join(tmpdir(), 'forge-migration-0027-'))
  ...
  try {
    ...
    if (sqlFiles.length !== baselineEntry.idx + 1) {
      throw new Error(`Expected ${baselineEntry.idx + 1} migration files through 0027; found ${sqlFiles.length}.`)
    }
  ...
  console.log('✓ Disposable upgrade database is at 0027.')
}
