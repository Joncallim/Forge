...
const PREDECESSOR_MIGRATION = '0035_verification_goal_execution_schema'
const TARGET_MIGRATION = '0036_verification_goal_runtime_routines'
...
async function main(): Promise<void> {
  ...
  if (!predecessor || !target || target.idx !== predecessor.idx + 1 || target.when <= predecessor.when) {
    throw new Error('0036 must immediately and chronologically follow 0035 in the migration journal.')
  }
  ...
  if (prefix.length !== target.idx + 1 || prefix.at(-1)?.tag !== TARGET_MIGRATION) {
    throw new Error('The exact migration journal prefix through 0036 is incomplete.')
  }
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'forge-migration-0036-'))
  ...
  console.log('✓ Disposable upgrade database is at 0036.')
}
