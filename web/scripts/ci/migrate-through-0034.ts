...
const TARGET_MIGRATION = '0034_verification_goal_executable_bindings'
...
async function main(): Promise<void> {
  ...
  if (!predecessor || !target || target.idx !== predecessor.idx + 1 || target.when <= predecessor.when) {
    throw new Error('0034 must immediately and chronologically follow 0033 in the migration journal.')
  }
  ...
  if (prefix.length !== target.idx + 1 || prefix.at(-1)?.tag !== TARGET_MIGRATION) {
    throw new Error('The exact migration journal prefix through 0034 is incomplete.')
  }
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'forge-migration-0034-'))
