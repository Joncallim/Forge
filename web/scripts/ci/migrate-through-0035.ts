...
const PREDECESSOR_MIGRATION = '0034_verification_goal_executable_bindings'
const TARGET_MIGRATION = '0035_verification_goal_execution_schema'
...
async function main(): Promise<void> {
  ...
  if (!predecessor || !target || target.idx !== predecessor.idx + 1 || target.when <= predecessor.when) {
    throw new Error('0035 must immediately and chronologically follow 0034 in the migration journal.')
  }
  ...
  if (prefix.length !== target.idx + 1 || prefix.at(-1)?.tag !== TARGET_MIGRATION) {
    throw new Error('The exact migration journal prefix through 0035 is incomplete.')
  }
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'forge-migration-0035-'))
  ...
  console.log('✓ Disposable upgrade database is at 0035.')
}
