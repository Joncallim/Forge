...
const PREDECESSOR_MIGRATION = '0027_epic_172_s4_packet_context'
const TARGET_MIGRATION = '0028_epic_172_s5_recovery_actions'
...
async function main(): Promise<void> {
  ...
  if (prefix.length !== target.idx + 1 || prefix.at(-1)?.tag !== TARGET_MIGRATION) {
    throw new Error('The exact migration journal prefix through 0028 is incomplete.')
  }
  const temporaryRoot = await mkdtemp(join(tmpdir(), 'forge-migration-0028-'))
  ...
  console.log('✓ Disposable upgrade database is at 0028.')
}
