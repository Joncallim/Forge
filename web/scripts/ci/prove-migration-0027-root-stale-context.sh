...
COMMIT;
SQL
expect_failure 'project-root task update has no active write context' psql "$url" --set ON_ERROR_STOP=1 --command "UPDATE public.tasks SET status='approved', error_message=NULL WHERE id='${task}'::uuid"
expect_failure 'project-root write context is absent or stale' psql "$url" --set ON_ERROR_STOP=1 --command "SELECT * FROM forge.complete_project_root_reconciliation_generation_v1('${operation_id}'::uuid,'${actor}'::uuid,1,'${project}'::uuid,'insert')"
expect_failure 'project-root task update has no active write context' psql "$url" --set ON_ERROR_STOP=1 --command "SELECT set_config('forge.project_root_context','forged',true); UPDATE public.tasks SET status='approved', error_message=NULL WHERE id='${task}'::uuid"
