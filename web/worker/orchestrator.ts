...
export function buildArchitectPrompt(
  ...
  return [
    ...
    '- When package review is needed, prefer explicit QA and/or Reviewer agents in `agent_breakdown_json`; Forge will run them as dependent sub-agent work packages after implementation instead of asking the operator to review every package manually. If no QA/Reviewer package is present, set each implementation agent `reviewRequirement` to the minimum manual review that work genuinely needs: `none` for trivial/low-risk changes (docs typos, config value tweaks), `qa_only` when functional verification matters but a human code review adds little, `reviewer_only` when code quality/design review matters but dedicated QA testing does not, or `both` for anything risky, security-sensitive, or touching shared/critical paths. Default to QA and Reviewer packages for normal code changes.',
    '',
